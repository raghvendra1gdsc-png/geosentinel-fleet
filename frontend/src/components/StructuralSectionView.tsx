...
export function StructuralSectionView({ missionState }: StructuralSectionViewProps) {
  ...
  const cfrpRef = useRef<HTMLDivElement>(null);
  const afterRef = useRef<HTMLDivElement>(null);

  useEffect(() => {
    const ctx = gsap.context(() => {
      if (!sectionRef.current || !pinRef.current || !beforeRef.current || !cfrpRef.current || !afterRef.current) return;

      // Set initial states
      gsap.set(cfrpRef.current, { opacity: 0.2, y: 30, scale: 0.95 });
      gsap.set(afterRef.current, { opacity: 0.2, y: 30, scale: 0.95 });
      ...
      // Phase 1 (0→0.33): Before is visible, CFRP fades in
      tl.to(cfrpRef.current, {
        opacity: 1,
        y: 0,
        scale: 1,
        duration: 0.33,
      ...
      // Phase 2 (0.33→0.66): CFRP is visible, After starts fading in
      ...
      // Phase 3 (0.66→1.0): Before fades slightly to emphasize the transformation
  ...
  return (
    <div ref={sectionRef} className="relative">
      <div ref={pinRef} className="bg-[#0b0c12] border border-white/10 rounded-2xl p-5 shadow-2xl relative overflow-hidden font-mono">
        {/* Header */}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-11 gap-4 items-center">
          ...
          <div ref={beforeRef} className="lg:col-span-4 bg-red-950/20 border border-red-500/40 p-4 rounded-xl flex flex-col justify-between h-full">
            <div>
          ...
          <div ref={cfrpRef} className="lg:col-span-3 bg-purple-950/20 border border-purple-500/40 p-4 rounded-xl flex flex-col justify-between h-full">
            <div>
          ...
          <div ref={afterRef} className="lg:col-span-4 bg-emerald-950/20 border-2 border-emerald-500/80 p-4 rounded-xl shadow-[0_0_20px_rgba(16,185,129,0.2)] flex flex-col justify-between h-full">
            <div>
